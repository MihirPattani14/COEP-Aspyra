...
def get_file_dir():
    id = request.cookies.get('key')
    updir = os.path.join(basedir, 'static/uploads/'+session[id]['mis']+'/store')
    return json.dumps({'dir':getfile.getdir(updir),'file':getfile.getfile(updir),'mis':session[id]['mis']})
...
def store_file():
    ...
    if id in session:
        ...
        if ext == 'AVI' or ext == 'avi' or ext == 'mkv' or ext == 'MKV' or ext == '3gp' or ext == '3GP' or ext == 'flv' or ext == 'FLV':
            files.save(os.path.join(updir, filename))
            t = threading.Thread(target=convert_file.convert,kwargs={'updir':updir,'filename':filename})
            t.start()
...
def se():
    print(session)
    return json.dumps({'s':str(session)})
...
def view_pdf():
    ...
    src = request.args.get('src')
    if id in session:
        if request.args.get('mis') == session[id]['mis']: #change here
...
def share_file():
    ...
    src = request.args.get('src')
    if id in session:
        if mis == session[id]['mis']: #change here
...
def view_video():
    ...
    src = request.args.get('src')
    if id in session:
        if request.args.get('mis') == session[id]['mis']: #change_here
...
def keep():
    ...
    src = request.args.get('src')
    if id in session:
        if src.split('/')[7] == session[id]['mis']: #change_here
            updir = os.path.join(basedir, 'static/uploads/'+session[id]['mis']+'/store/')
            subprocess.call(['mv',src,updir])
            updir = os.path.join(basedir, 'static/uploads/'+session[id]['mis']+'/share/')
            try:
                os.rmdir(updir+src.split('/')[9]) #change_here
...
def delete_folder():
    ...
    src = request.args.get('src')
    if id in session:
        if request.args.get('mis') == session[id]['mis']: #changehere
            subprocess.call(['rm','-rf',src])
            return render_template('drive_download .html')
        return render_template('error.html')
    return render_template('no_login.html')

@app.route('/learn_linux')
@nocache
def learn_linux():
	id = request.cookies.get('key')
	if id in session:
...
@app.route('/learn_deploy', methods=['POST'] )
@nocache
def learn_dep():
    ports = getPort(1)
    port = str(ports[0])
    from subprocess import call
    call("docker run -p "+port+":80 -d drupal", shell=True)
    ip = "http://0.0.0.0:" + port + "/"
    return json.dumps({'status':"success", 'port': port})


if __name__ == '__main__':
    app.run(host='0.0.0.0',port=14000,processes = 5,debug=True,use_reloader=False)
